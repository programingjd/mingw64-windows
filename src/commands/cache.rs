...
use crate::commands::utils::{file_was_updated_recently, yes_or_no};
...
fn get_available_packages(available_packages_file: &Path) -> BTreeSet<Package> {
    ...
    let mut cache_versions = File::open(available_packages_file)
    ...
    if repositories_to_sync.is_empty() {
        ...
        if repositories_to_sync.len() == Repository::enabled().len() {
                            ...
                            let other_repositories: BTreeSet<&Repository> = Repository::enabled()
                                ...
                                .filter_map(|&repository| {
                                    if successfully_synced_packages
                                        .iter()
                                        .any(|it| it.0.repository == repository)
                                    {
                                ...
                                .collect();
                            let mut other_repository_packages: BTreeMap<
                                &Repository,
                                (RepositoryVersion, Vec<Package>),
                            > = BTreeMap::new();
                            packages
                                .into_iter()
                                    ...
                                    other_repository_packages
                                        ...
                                        .or_else(|| {
                                            cache_versions
                                                ...
                                                .and_then(|it| Some((it.clone(), Vec::new())))
                                        });
                                });
            ...
            if let Ok(packages) = get_available_packages_from_file(available_packages_file) {
                ...
                let mut full_list: Vec<(RepositoryVersion, Vec<Package>)> = Vec::new();
                            ...
                            None => {
                                ...
                                if let Some(values) = package_map.remove(repository) {
                                    if let Some(cache_version) = cache_versions.get(repository) {
                                        full_list.push((cache_version.clone(), values))
                                    }
                                }
                                None
                            }
...
fn save_and_return_available_packages(
    ...
    repository_packages: Vec<(RepositoryVersion, Vec<Package>)>,
) -> BTreeSet<Package> {
...
fn save_available_packages(
    ...
    let mut header = repository_packages
        .iter()
        .map(|it| format!("{} {}", it.0.repository.name(), &it.0.etag))
        .collect::<Vec<_>>()
        .join(" ");
    header += "\n";
    encoder.write_all(&header.as_bytes())?;
    // for each repo, add 1 line for each package
    repository_packages.iter().for_each(|repo| {
        repo.1.iter().for_each(|package| {
